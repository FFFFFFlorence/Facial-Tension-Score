...
plt.subplots_adjust(bottom=0.32, right=0.70)  # room for Start/Stop, timeframe selector, summary text, voice panel, and the gauge

line, = ax.plot([], [], color='black', linewidth=1.6, zorder=5)
ax.set_xlabel("Time (s)")
ax.set_ylabel("Tension Score (relative to baseline)")
ax.set_title("Live Tension Monitor - PROTOTYPE (click Start to begin)")
...
def on_timeframe_change(label):
    ...
    conclusion_text.set_text("Waiting for enough data in the new timeframe...")
    fig.canvas.draw_idle()


radio_ax = plt.axes([0.73, 0.55, 0.13, 0.15])
...
# ---------------------------------------------------------------------------
voice_panel_ax = plt.axes([0.73, 0.20, 0.13, 0.28])
...
# ---------------------------------------------------------------------------
# Expression/tension summary text - recalculated once per full timeframe window
# ---------------------------------------------------------------------------
conclusion_text = fig.text(
    0.1, 0.08,
    "Summary will appear here once a full timeframe window has elapsed.",
...
def update_conclusion():
    ...
    if mean_val >= HIGH_THRESHOLD:
        level = "HIGH tension pattern"
    elif mean_val >= ELEVATED_THRESHOLD:
        level = "ELEVATED tension pattern"
    elif mean_val <= -ELEVATED_THRESHOLD:
        level = "BELOW BASELINE (calmer than neutral)"
    else:
        level = "WITHIN normal baseline range"

    variability_note = ""
    if std_val >= HIGH_VARIABILITY_THRESHOLD:
        variability_note = " Scores fluctuated notably during this window."

    text = (
        f"[Last {selected_timeframe_seconds}s] {level} (avg {mean_val:+.2f}, variability {std_val:.2f}).{variability_note}\n"
        f"Heuristic pattern summary only - not a determination of nervousness, honesty, or intent."
    )
    conclusion_text.set_text(text)
